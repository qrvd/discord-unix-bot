...
async def on_ready():
    u = pawn.user
    with open('var/bot/pawn.id', 'w') as f:
        f.write(str(u.id))
    with open('var/bot/pawn.name', 'w') as f:
...
async def speak():
        ...
        try:
            while True:
                ...
                if channel == None:
                    ...
                    with open('var/pawn-error.log', 'a') as f:
                        f.write(str(errmsg) + '\n')
                    raise errmsg
    ...
    server = await asyncio.start_server(
        onspeak, cat('etc/bot/pawn.addr'), int(cat('etc/bot/pawn.port'))
    )
...
pawn.run(cat('etc/bot/pawn.token'))
